...
app.get("/greenhouses_crops", function (req, res) {
  ...
  let query1 =
    "SELECT Greenhouses_Crops.greenhouse_crop_id, Greenhouses_Crops.greenhouse_id, Greenhouses_Crops.crop_id, Crops.crop_name FROM Greenhouses_Crops INNER JOIN Crops ON Greenhouses_Crops.crop_id = Crops.crop_id;";
        ...
        if (error) {
          ...
          db.pool.query(query3, (error, rows, fields) => {
            if (error) {
              ...
              let greenhouses = rows;
              res.render("greenhouses_crops", {
                data: greenhouses_crops,
                crops: crops,
                greenhouses: greenhouses,
              });
            }
...
app.put("/put-greenhouses-crops-ajax", function (req, res, next) {
  ...
  db.pool.query(
    queryUpdateGreenhousesCrops,
    [cropID, greenhouseID, greenhouseCropID],
    function (error, rows, fields) {
      if (error) {
...
app.post("/add-trades", function (req, res) {
  ...
  let vault_id =
    !data.vault_id || data.vault_id === "" ? "NULL" : parseInt(data.vault_id);
  let resource_id =
    !data.resource_id || data.resource_id === ""
      ? "NULL"
      : parseInt(data.resource_id);
  let crop_id =
    !data.crop_id || data.crop_id === "" ? "NULL" : parseInt(data.crop_id);
...
app.put("/put-trades-ajax", function (req, res, next) {
  ...
  let resource_id =
    data.resource_id === null ? "NULL" : parseInt(data.resource_id);
